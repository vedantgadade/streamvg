...
const escUrl=u=>API_BASE+'/api/proxy?url='+encodeURIComponent(u);const teraHosts=['terabox.com','www.terabox.com','terabox.app','www.terabox.app','teraboxshare.com','teraboxlink.com','terafileshare.com','terasharefile.com','terasharelink.com','1024terabox.com','1024tera.com'];const isTeraBox=u=>{try{return teraHosts.includes(new URL(u).hostname.toLowerCase())}catch{return false}};const resolveTeraBox=async u=>{const x=new URL(u);let code=x.searchParams.get('surl')||'';if(!code){const parts=x.pathname.split('/').filter(Boolean);const s=parts.indexOf('s');if(s>=0)code=parts[s+1]||''}code=code.replace(/^1/,'');if(!code)throw new Error('Invalid TeraBox share link');return 'https://tbx-proxy.shakir-ansarii075.workers.dev/?mode=stream&surl='+encodeURIComponent(code)+'&type=M3U8_AUTO_720'};const isYouTube=u=>{try{const h=new URL(u).hostname.toLowerCase();return h==='youtube.com'||h==='www.youtube.com'||h==='m.youtube.com'||h==='youtu.be'}catch{return false}};const youtubeEmbed=u=>{const x=new URL(u);let id=x.searchParams.get('v')||'';const parts=x.pathname.split('/').filter(Boolean);if(!id&&parts.length>=2&&(parts[0]==='shorts'||parts[0]==='embed'||parts[0]==='live'))id=parts[1];if(!id&&x.hostname.toLowerCase()==='youtu.be')id=parts[0]||'';return id?'https://www.youtube.com/embed/'+encodeURIComponent(id)+'?autoplay=1&rel=0&modestbranding=1':null};
